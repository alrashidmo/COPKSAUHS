...
serve(async (req) => {
  ...
  try {
    ...
    if (!["approved", "rejected"].includes(record.status)) {
      return new Response("Status not actionable", { status: 200, headers: corsHeaders });
    }

    const studentEmail = record.student_email;
    if (!studentEmail) {
      return new Response("No student email on record", { status: 200, headers: corsHeaders });
    }
